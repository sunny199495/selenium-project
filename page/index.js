...
data.browsers.forEach((browser, index) => {
  describe("login and logout", function () {
    ...
    async function goto() {
      ...
      // 找到用户名和密码输入框
      username = await driver.findElement(data.input.username);
      password = await driver.findElement(data.input.password);
      // 清空输入框
    ...
    async function login() {
      // 输入用户名和密码
      await username.sendKeys(data.user.username);
      await password.sendKeys(data.user.password);
      // 点击登录
